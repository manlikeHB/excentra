...
pub async fn create_order(pool: &PgPool, order: DBOrder) -> Result<DBOrder, sqlx::Error> {
    ...
    (user_id, pair_id, side, order_type, price, quantity, remaining_quantity, status) 
    VALUES ($1, $2, $3::order_side, $4::order_type, $5, $6, $7, $8::order_status) 
    RETURNING 
    id, 
    user_id, pair_id, 
...
pub async fn get_user_orders(pool: &PgPool, user_id: Uuid) -> Result<Vec<DBOrder>, sqlx::Error> {
    ...
    WHERE user_id = $1"#, user_id).fetch_all(pool).await
}
